export default (request) => {
    ...
    const query_params = {
        ...
        "apikey": "32078acd401f17cc7094c83c4137b73d412e1894", //new standard plan
    ...
    function parseMessage(msg) {
        ...
        return xhr.fetch(url).then((x)=>{
            ...
            if(ret.language != 'english') {
                ...
                return translateMessage(msg, ret.language);
            }
    ...
    function translateMessage(msg,lang) {
        ...
        return xhr.fetch(url, http_options).then((x) => {
            ...
            //parseMessage({text: x.body, originalLanguage:lang});
            var msgg = {
                original: x.body,
                parsed:null,
                originalLanguage:lang
            };
            console.log("forwarded on to cook-message",msgg);
            pubnub.publish({channel:'cook-message',message:msgg});
            return request.ok();

            //pubnub.fire({channel:'chatbot',message:{text:x.body, originalLanguage:lang}});
            return request.ok();
        });
    }

    return parseMessage(request.message);
